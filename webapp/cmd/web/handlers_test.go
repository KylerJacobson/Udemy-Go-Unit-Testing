...
func Test_application_handlers(t *testing.T) {
	var tests = []struct {
		...
		{"404", "/fern", http.StatusNotFound, "/fern", http.StatusNotFound},
	...
	for _, e := range tests {
		...
		resp2, _ := client.Get(s.URL + e.url)
...
func Test_app_Login(t *testing.T) {
	...
	for _, e := range tests {
		...
		if err == nil {
			...
			t.Errorf("%s: no location header set in the test", e.name)
		}
...
func Test_app_UploadFiles(t *testing.T) {
	...
	// create a new writer, of time *io.Writer

	writer := multipart.NewWriter(pw)

	// create a waitgroup and add one 1 to it
